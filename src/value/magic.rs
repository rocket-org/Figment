...
use crate::{
    value::{ConfiguredValueDe, MapDe, Tag},
    Error,
};
        ...
        if let Some(d) = de.value.as_dict() {
            if let Some(mpv) = d.get(Self::FIELDS[0]) {
                if mpv.to_empty().is_none() {
                    return visitor
                        .visit_map(MapDe::new(d, |v| ConfiguredValueDe::from(config, v)));
                }
            }
        }
        ...
        if let Some(dict) = de.value.as_dict() {
            if let Some(tagv) = dict.get(Self::FIELDS[0]) {
                if let Ok(false) = tagv.deserialize::<Tag>().map(|t| t.is_default()) {
                    return visitor
                        .visit_map(MapDe::new(dict, |v| ConfiguredValueDe::from(config, v)));
                }
            }
        }
...
mod _serde {
    ...
    const _: () = {
            ...
            B: _serde::Serialize,
        {
            ...
            {
                match *self {
                    Either::Left(ref __field0) => _serde::Serializer::serialize_newtype_variant(
                        __serializer,
                        "Either",
                        0u32,
                        "Left",
                    ...
                    Either::Right(ref __field0) => _serde::Serializer::serialize_newtype_variant(
                        __serializer,
                        "Either",
                        1u32,
                        "Right",
